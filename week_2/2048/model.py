...
def expectimax(b, direction_values, turn, depth=0):
    ...
    if turn == True:
        ...
        for func in MERGE_FUNCTIONS.keys():
            new_board = MERGE_FUNCTIONS[func](b)
            ul = upper_left(new_board)
            es = empty_spaces(new_board)
            mv = max_value(new_board)
            # print("ul for {0} is {1}".format(func, ul))
            # print("es for {0} is {1}".format(func, es))
            # print("mv for {0} is {1}".format(func, mv))
            direction_values[func] += (mv + ul) * es
            # print("Direction values : {0}".format(direction_values))
            direction_values = expectimax(new_board, direction_values, False, depth + 1)
    else: 
        new_board = add_two_four(b)
        direction_values = expectimax(new_board, direction_values, True, depth + 1)
...
def get_expectimax_move(b):
    ...
    for x in range(4):
        print(b[x])
    direction_values = expectimax(b, direction_values, True)
