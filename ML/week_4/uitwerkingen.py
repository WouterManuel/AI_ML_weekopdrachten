...
def drawGraph(data):
    ...
    x, y = data.T
    plt.scatter(x,y)
    plt.title('Winst van vervoerder op basis van stadspopulatie')
    plt.xlabel('Populatie (x1000)')
    plt.ylabel('Winst vervoerder (x1000)')
...
def computeCost(X, y, theta):
    ...
    # for i in range(0, m):
    #     delta = 0
    #     hypothese = theta[0] + theta[1] * X[1]
    #     delta += (hypothese - y[i]) ** 2 
    #     J += delta
    # J = J / (2*m)
...
def contourPlot(X, y):
    ...
    # twee dimensionale arrar [[]...[]] van x lengte
    J_vals = np.zeros( (len(t2), len(t2)) )

    for i in range(len(t1)):
        for j in range(len(t2)):
            J_vals[i][j] = computeCost(X, y, [t1[i], t2[j]])
